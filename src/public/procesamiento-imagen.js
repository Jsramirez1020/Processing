...
read("imagen.png")
  .then((img) => {
    ...
    const imgMatrix = matrix(img.bitmap.data, [
      img.bitmap.height,
      img.bitmap.width,
    ]);
    ...
    forEach(resultMatrix, (value, index) => {
      if (
        index[0] > 0 &&
        index[0] < imgMatrix.size()[0] - 1 &&
        index[1] > 0 &&
        index[1] < imgMatrix.size()[1] - 1
      ) {
        const subMatrix = subset(
          imgMatrix,
          _index(
            range(index[0] - 1, index[0] + 2),
            range(index[1] - 1, index[1] + 2)
          )
        );
    ...
    const resultImg = new Jimp({
      data: resultBuffer,
      width: resultMatrix.size()[1],
      height: resultMatrix.size()[0],
    });
  ...
  () => {
    ...
    reader.onload = () => {
      ...
      img.onload = () => {
        ...
        jimpImg.bitmap.data = new Buffer.from(
          img.src.replace(/^data:image\/png;base64,/, ""),
          "base64"
        );

        // Convertir la imagen en una matriz de Math.js
        const imgMatrix = matrix(jimpImg.bitmap.data, [
